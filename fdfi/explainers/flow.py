    ...
    **kwargs : dict
    ...
    def explain_batches(
        ...
        **kwargs: Any,
        ...
        The batching path uses the same normalized CPI and SCPI definitions as
        :meth:`__call__`. When ``y`` is supplied, each batch receives the
        matching response slice for label-aware loss differences.
        """
        X = np.asarray(X)
        if X.ndim != 2:
