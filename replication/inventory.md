...
- `figures/d3_simulation_runtime.pdf`: reviewed Figure D3 server reproduction
  for CPI, LOCO, nLOCO, dLOCO, OT, EOT, FDFI, and SHAP on a log-second axis,
  using 16 successful repetitions at n=200, 400, 600, 800, and 1000;
- feature-level, runtime, benchmark-summary and runtime-summary CSV sources.

The manuscript D3 artifact above is generated by `reproduce_d3.py`; its
committed numerical sources are `results/d3_simulation_runtime_results.csv`
and `tables/d3_simulation_runtime_summary.csv`.
The raw checkpoint contains seven additional successful `n=200` cells from an
incomplete 17th repetition. These rows are retained as checkpoint provenance;
the reviewed summary and PDF use only the 16 complete repetitions.
