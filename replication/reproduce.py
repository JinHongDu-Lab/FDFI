#!/usr/bin/env python3
"""Run the complete FDFI JSS replication workflow."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reproduce all computational results in the FDFI JSS paper."
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="run/check one workflow for development; the plain full command remains strict",
    )
...
if __name__ == "__main__":
    args = parse_args()
    raise SystemExit(run_replication(mode="full", check_only=args.check, workflow=args.workflow))
